...
class FreeUpload extends FileUpload implements HasEmbeddedView
{
    ...
    {
        $endpoint = trim((string) config('free-upload.upload_endpoint', ''));
        ...
        if (str_starts_with($endpoint, '/') || str_contains($endpoint, '/')) {
            return $endpoint;
        }
                        ...
                        uploadUsing: (fileKey, file, success, error, progress) => {
                            ...
                            const send = (blob, encoded) => {
                                ...
                                xhr.addEventListener('load', () => {
                                    ...
                                    const current = $wire.get(<?= Js::from($statePath) ?>)
                                    const next = Array.isArray(current) && current.includes(data.url)
                                        ? current
                                        : [...(Array.isArray(current) ? current : []), data.url]

                                    $wire.set(<?= Js::from($statePath) ?>, next)
                                })
